...
def extract_indeed_pages(url):
    ...
    if search_count_pages:
        number_jobs_searched_string = search_count_pages.string.split(
        )[-1][:-1]

        number_jobs_searched = convert_string_with_comma_to_number(
            number_jobs_searched_string)
